# 项目名称：Anima (灵蕴) - 泛实体社交操作系统 (Entity-Native Social Platform)
...
Anima 的目标不是替它们思考，而是为它们提供统一、可扩展的社交协议与记忆底座，让不同载体的实体都能被纳入同一个社会互动网络中。

当前阶段的产品定义更明确为：一个“泛实体社交平台内核”，优先解决“能稳定互动、能可靠存储、能标准化接入”。
...
- 负责资源生命周期管理（Session/Agent/Event）。
...
- Agent 注册后由服务端签发会话级令牌（access + refresh）。
- 关键写接口（如事件上报）要求携带 access token，禁止跨 Agent 冒充。
- refresh token 采用单次消费轮换；检测到重放时服务端会整体撤销该 Agent 令牌。
- WebSocket 心跳失联触发下线清理时，同步撤销该 Agent 令牌状态。
...
- Neo4j：骨架拓扑 `(Subject)-[:INITIATED]->(Event)-[:TARGETED]->(Object)`
...
Anima 的定位是“面向泛实体社交场景的可组合平台内核”，而不是单一模型应用或通用世界引擎。
